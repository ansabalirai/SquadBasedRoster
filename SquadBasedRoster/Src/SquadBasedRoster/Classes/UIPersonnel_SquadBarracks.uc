...
class UIPersonnel_SquadBarracks extends UIPersonnel config(SquadBasedRoster);
...
{
	return class'XComGameState_SBRSquadManager'.static.GetSquadManager().Squads.Find('ObjectID', SquadRef.ObjectID);
}
...
simulated function OnImageScrollButtonClicked(UIButton Button)
{
	...
	Squad = XComGameState_SBRSquad(NewGameState.CreateStateObject(class'XComGameState_SBRSquad', Squad.ObjectID));
	Squad.SquadLogoPath = SquadImagePaths[CurrentIndex];
...
simulated function XComGameState_SBRSquad GetCurrentSquad()
{
...
simulated function bool CanTransferSoldier(StateObjectReference UnitRef, optional XComGameState_SBRSquad SquadState)
{
	...
	// TODO: Can't move soldiers that are on a mission (this does not include haven liaisons). Probably define IsUnitOnMission() to take into account CI
	...
	if(SquadState != none)
	{
		...
		MaxSize = class'XComGameState_SBRSquadManager'.default.MAX_SQUAD_SIZE;
		if(NumSoldiers >= MaxSize)
			if(bViewUnassignedSoldiers)
				return false;


		// The restriction to not allow transferring of squad leader or cross-faction specialists is
		// accounted for in CanSoldierBeAdded and CanSoldierBeRemoved
...
simulated function UpdateSquadHeader()
{
	...
	else
	{
		...
		ParamTag.StrValue0 = SquadState.SquadName;
	...
	{
		ParamTag = XGParamTag(`XEXPANDCONTEXT.FindTag("XGParam"));
		ParamTag.IntValue0 = SquadState.NumMissions;
		MissionsString = `XEXPAND.ExpandString(strMissions);
		SquadMissionsText.SetText(MissionsString);
		SquadBiography.SetHTMLText(SquadState.SquadBiography);
	}
}
...
simulated function UpdateSquadList()
{
	...
	SquadMgr = class'XComGameState_SBRSquadManager'.static.GetSquadManager();
		// Rai - Inserting a squad status update here
...
simulated function OnSoldierSelected( UIList kList, int index )
{
	...
	// Rai - Add additional check here if the soldier can be added to the squad as per SBR rules
	// To do: Maybe Some better handling here to ensure that faction leaders can only be swapped?
	// if (!SquadState.CanSoldierBeAdded(UnitState.GetReference()))
	// 	return;

	NewGameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("Transferring Soldier");
	SquadState = XComGameState_SBRSquad(NewGameState.CreateStateObject(class'XComGameState_SBRSquad', SquadState.ObjectID));
	NewGameState.AddStateObject(SquadState);

	if(bViewUnassignedSoldiers) // Adding a new solider to squad from barracks
	{
		if (SquadState.CanSoldierBeAdded(UnitState.GetReference()))
			SquadState.AddSoldier(UnitState.GetReference());
	}
	else // Removing an existing solider from squad to return to pool
	{
		if (SquadState.CanSoldierBeRemoved(UnitState.GetReference()))
...
function OnRenameClicked(UIButton Button)
{
	...
	kData.strInputBoxText = GetCurrentSquad().SquadName;
...
function OnEditBiographyClicked(UIButton Button)
{
	...
	if(CurrentSquadSelection >= 0)
	{
		...
		kData.strInputBoxText = GetCurrentSquad().SquadBiography;
...
simulated function OnDeleteSquadDialogCallback(Name eAction)
{
	...
	{
		SquadRef = UISquadListItem(m_kSquadList.GetItem(CurrentSquadSelection)).SquadRef;
		class'XComGameState_SBRSquadManager'.static.GetSquadManager().RemoveSquadByRef(SquadRef);
		CurrentSquadSelection = -1;
		RefreshAllData();
	}
}
...
{
	class'XComGameState_SBRSquadManager'.static.GetSquadManager().CreateEmptySquad();
...
function OnEditOrSelectClicked(UIButton Button)
{
	...
	if(bSelectSquad)
	{
		if(SquadState != none && !SquadState.IsDeployedOnMission())
		{
			...
			SquadMgr = class'XComGameState_SBRSquadManager'.static.GetSquadManager();
			UpdatedSquadMgr = XComGameState_SBRSquadManager(UpdateState.CreateStateObject(SquadMgr.Class, SquadMgr.ObjectID));
...
simulated function CloseScreen()
{
	...
	SquadSelect = class'XComGameState_SBRSquadManager'.static.GetSquadManager().GetSquadSelect();
	if (SquadSelect != none)
	{
		SquadContainer = UISquadContainer(SquadSelect.GetChildByName('SquadSelect_SquadContainer_LW', false));
	}
