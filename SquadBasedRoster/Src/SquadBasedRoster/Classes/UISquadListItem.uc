...
var localized string sSquadOnMission, sSquadAvailable, sAffinity;
...
simulated function Update()
{
	...
	local string AffinityString, SquadLevelString;
	local string AffinityValString, SquadLevelValString;
	...
	SquadName = SquadState.SquadName;
	SquadNameText.SetTitle(SquadName, bIsFocused, true);
	
	// Rai - Update the squad affinity and other details here, since we are refreshing after adding/removing someone
	...
	// Rai - To Do - Add affinity and squad level information here and also try to format soldier icons to show leader, specialists and solidiers more clearly

	AffinityValString = class'X2Helper_SquadBasedRoster'.static.TruncFloat(SquadState.GetAverageAffinity(SquadState.SquadSoldiers),1);
	AffinityString = "AA: " $ AffinityValString;
	SquadLevelValString = class'X2Helper_SquadBasedRoster'.static.TruncFloat(SquadState.SquadLevel,1);
	SquadLevelString = " , SL: " $ SquadLevelValString;
	SquadCovertnessText.SetHTMLText(class'UIUtilities_Text'.static.AlignRight(AffinityString $ SquadLevelString));
...
simulated function int GetClassIconAlphaStatus(XComGameState_Unit UnitState, XComGameState_SBRSquad SquadState)
{
